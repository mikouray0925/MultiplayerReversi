...
public class ReversiChessSpawner : MonoBehaviour
{
    ...
    public delegate GameObject InstantiateMethod(Object original, Vector3 position, Quaternion rotation, Transform parent);
    public Dictionary<string, ReversiChess> SpawnChesses(InstantiateMethod instantiateMethod) {
        if (chessPrefab) {
            ...
            for (int row = 1; row <= 8; row++) {
                for (char col = 'A'; col <= 'H'; col++) {
                    ...
                    ReversiChess chess = instantiateMethod(chessPrefab, currentPos, Quaternion.identity, transform).GetComponent<ReversiChess>(); 
